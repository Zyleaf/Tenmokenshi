...
class Poke extends BaseCommand {
    ...
    run = async () => {
        ...
        async function getGif() {
            ...
            message.channel.send(embedBuilder(true, false, `${message.author.username} pokes ${user.username}!`, false, false, false, false, url.url, true, true));
        }
        getGif();
    }
}
...
module.exports = {
    ...
    execute: (client, message, permissions, args, parsedArgs, requiredArgs, cooldown, usage, name, sql) => {
        let command = new Poke(client, message, permissions, args, parsedArgs, requiredArgs, cooldown, usage, name, sql);
        command.checkPerms();
    }
};